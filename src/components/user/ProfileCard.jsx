...
theme="light"  />
      ...
      <ImageHover handler={handleImage} photo={user.photo} />
      </div>    
      <div className="w-10/12 mx-auto pt-5 px-5">
      <h1 className='font-Poppins text-[15px] text-gray-500 flex justify-start items-center   my-2 text-left'><i class='mr-2 bx bxs-user-pin'></i> {user?.name}</h1>
            <h1 className='font-Poppins text-[15px] text-gray-500 flex justify-start items-center   my-2 text-left'>
            <i class='bx bxl-gmail pr-2 '></i> {user?.email}</h1>
            <h1 className='cursor-pointer font-Poppins text-sm text-gray-500 flex justify-start items-center   my-2 text-left'>
            <i class='bx bxs-file-doc pr-2 text-red-500' ></i> {user?.certificate==='no-file'?
            <UploadFile label={"Upload Certificate"} onChange={handleCertificate} accept={".pdf"}></UploadFile>  :        <span onClick={()=>{
